    ...
    <form action="{{ isset($referral) ? route('referrals.update', $referral) : route('referrals.store') }}" 
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="mb-4">
                ...
                <select name="destination_hospital_id" id="destination_hospital_id" 
                        class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" 
                        required>
            ...
            <div class="mb-4">
                ...
                <textarea name="keadaan_saat_rujuk" id="keadaan_saat_rujuk" 
                          class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" 
                          rows="3">{{ isset($referral) ? $referral->keadaan_saat_rujuk : old('keadaan_saat_rujuk') }}</textarea>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        patientSelect.addEventListener('change', function() {
            if (this.value) {
                fetch(`/referrals/patient-details?patient_id=${this.value}`)
                    ...
                    .then(data => {
                        genderInput.value = data.gender === 'male' ? 'Laki-laki' : 'Perempuan';
                ...
                // Clear inputs if no patient is selected
                genderInput.value = '';
                addressInput.value = '';
        ...
        // Populate fields if editing an existing referral
        @if(isset($referral))
            genderInput.value = "{{ $referral->gender }}";
            addressInput.value = "{{ $referral->address }}";
            hasilPemeriksaanInput.value = "{{ $referral->hasil_pemeriksaan }}";
        @endif
    });
</script>
@endpush